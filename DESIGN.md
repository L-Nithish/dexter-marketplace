...
Dominate's commerce system is built on a single, almost violently simple idea: photography speaks, the chrome doesn't. Every page reads as a marketing editorial — towering uppercase display campaign lockups (`{typography.display-campaign}`) burned into full-bleed workspace and interface imagery, with everything else (nav, filters, buttons, cards, footer) reduced to neutral typography and pill geometry on `{colors.canvas}` and `{colors.soft-cloud}`. There is no decorative gradient, no soft shadow nostalgia, no accent color used for "tone" — the system saves all chromatic energy for digital product mockups, data dashboards, and the small handful of moments that actually need to signal (sale price `{colors.sale}`, success `{colors.success}`, swatch dots).

The result is a layout that feels physical — campaign hero, product grid, digital resource tile, footer — stacked like a printed catalog rather than animated like a typical SaaS landing page. Density is high but never crowded, because the system relies on three relentless devices: square or near-square 1:1 product imagery on `{colors.soft-cloud}`, pill-shaped black CTAs (`{rounded.full}`) anchoring every actionable surface, and a tight 8px-base spacing scale that keeps cards and filters mathematically aligned across PLP, PDP, and editorial pages.

Across `/courses`, the SEO-masterclass listing, the Meta Ads Blueprint PDP, `/membership`, and automation workflows, the same chrome appears in identical proportions — only the photography and copy change. That is the system's signature: maximum editorial expression in the imagery, maximum mechanical restraint everywhere else.

**Key Characteristics:**
- Editorial campaign hero with `{typography.display-campaign}` (Dominate Display, 96px, line-height 0.9, uppercase) burned directly into full-bleed photography
...
- Product cards have zero radius, zero shadow, sit directly on `{colors.soft-cloud}` swatch backgrounds — the mock-up image is the card
...
> **Source pages:** `/courses` (primary), `/w/templates-canva-social-packs-…`, `/t/meta-ads-blueprint-…`, `/membership`, `/w/automation-workflows-…`. The chrome palette is identical across all five — only photography varies.

### Brand & Accent
- **Dominate Black** (`{colors.ink}` — `#111111`): The brand's only "color." It is the primary CTA, the swatch dot, the active filter chip, the campaign overlay, the headline color, and the body text. When Dominate wants to assert anything, it goes black.
...
### Surface
- **Soft Cloud** (`{colors.soft-cloud}` — `#f5f5f5`): The most-used non-white surface in the entire system. Product card image backgrounds, search pill, secondary CTA, utility bar, category swatch tiles. It is the "color" of every product mockup's stage.
...
- **Mute** (`{colors.mute}` — `#707072`): Product category subtitles ("Advanced Meta Ads Course"), footer link text, secondary metadata.
...
- **Success** (`{colors.success}` — `#007d48`): Confirmation messages, order success indicators, eligibility ticks.
...
### Category Accents (domain / track chips)
These appear sparingly — almost exclusively as small chip backgrounds, swatch dots, or track illustrations in editorial tiles. They are never used as text or primary CTA color.
- **Accent Pink** (`{colors.accent-pink}` — `#ed1aa0`): Meta Ads / copywriting collection moments.
...
- **Accent Teal** (`{colors.accent-teal}` — `#0a7281`): SEO / data analytics / coding editorial accent in lockups.
- **Accent Pink Deep** (`{colors.accent-pink-deep}` — `#4c012d`): Deepest editorial overlay tint, used as wash on agency masterclasses / automation tiles.

## Typography

### Font Family
- **Dominate Display** (display campaign only) — proprietary geometric sans for the towering uppercase headlines burned into campaign hero photography. Falls back to Bebas Neue → Helvetica Now Text Medium → Helvetica → Arial.
...
When substituting on systems without proprietary Dominate fonts: pair **Inter** (Display 700 for body chrome, Display 500 for buttons) with **Bebas Neue** or **Anton** at 96px/0.9 line-height for the campaign headline tier. Tighten letter-spacing slightly (-0.5%) on the substitute to approximate Dominate Display's optical weight.
...
| `{typography.heading-xl}` | 32px | 500 | 1.2 | 0 | Section headers — "BEST SELLERS", "AI AUTOMATIONS", PDP product title block |
...
| `{typography.button-sm}` | 14px | 500 | 1.5 | 0 | Compact pill CTA, badge label, location-selector button |
| `{typography.link-md}` | 16px | 500 | 1.75 | 0 | Underlined inline link, "View Product Details" |
| `{typography.caption-md}` | 14px | 500 | 1.5 | 0 | Product subtitle ("Advanced Ads Course"), filter count, footer link |
| `{typography.caption-sm}` | 12px | 500 | 1.5 | 0 | Filter chip label, badge text, formats count |
| `{typography.utility-xs}` | 9px | 500 | 1.75 | 0 | Legal copyright / fine-print row at the very bottom |

### Principles
The system runs on extreme typographic contrast: a single 96px uppercase display tier reserved for editorial campaign moments, and a quiet 12–16px Helvetica Now Text/Medium tier carrying everything else. There is almost no middle ground — the jump from `{typography.heading-xl}` (32px) directly to `{typography.body-strong}` (16px) is intentional and creates the "billboard above, catalog below" effect across every page. Letter-spacing is left at 0 (Dominate Display and Helvetica Now are both cut for tight optical fit at scale).

### Note on Font Substitutes
The closest open-source path to Dominate's display tier is **Bebas Neue** (free, geometric condensed) at 96px / 0.9 / uppercase / 500. For UI text, **Inter** is the safest substitute — match weights 400/500 and the system reads almost identically at button and caption sizes.
...
- **Column patterns:** PLP listing uses 3-up at desktop, collapsing to 2-up at 1023px and 1-up at 599px. The marketplace home mixes a 2-up campaign hero row, a 3- or 4-up "Trending Now" row, a horizontal-scroll "Shop by Category" rail, and a 4-up "Latest in Assets" thumbnail grid.
...
### Decorative Depth
Depth in Dominate's system comes entirely from photography, not from CSS effects:
- **Editorial campaign tiles** create depth via cinematic perspective — a marketer in a modern workspace, code scripts and analytics overlays — with the display campaign headline overlaid in white or `{colors.ink}` directly on the image.
- **Product card photography** is shot on flat `{colors.soft-cloud}` to remove any background depth, so the digital mockup itself is the only thing with form on the page.
- **Category tiles** on the home page are full-bleed workspace photography with a small `{component.button-outline-on-image}` pill anchored at the bottom-left, giving a moment of crisp white pill against atmospheric image.
...
| `{rounded.lg}` | 30px | Every CTA pill — primary, secondary, on-image, filter chip, geo-selector, "Get Access" |
| `{rounded.full}` | 9999px | Color/Format swatch dots and circular icon buttons (back, share, favorite, carousel paddle) |
...
- **Editorial campaign hero:** ~16:9 or wider cinematic crop, full-bleed across the content max-width, with the display campaign headline burned into the lower-left or upper-left third.
- **Category rail:** 4:5 portrait full-bleed thumbnails with a small CTA pill anchored bottom-left.
- **PDP main image:** square primary image with vertical thumbnail rail to its left (~5–7 thumbnails stacked at small size), enabling rapid details/dashboards browsing without leaving the page.
...
**`button-primary`** — the universal Dominate CTA
...
- Used on every primary action in the system: "Add to Bag", "Get Access", "Instant Download", "Confirm Location" geo-confirm, "Shop" overlay on category tiles, "Continue".
- Pressed state lives in `button-primary-active` — the bg stays `{colors.ink}` while the surface shrinks to `scale(0.5)` with `opacity: 0.5` (Dominate's signature "tap collapse" feedback).
...
- Used as the lower-emphasis alternate when a primary CTA already exists, e.g., "Cancel" or "Discover More" on light cards.
...
- The crisp white pill that anchors the bottom-left of every full-bleed category and editorial campaign tile.
...
- Image area: `{component.product-card-image}` — full-bleed product mockup/photo on `{colors.soft-cloud}` square.
- Below image (in this order with `{spacing.sm}` between): swatch dot row (3–6 dots at 12px circular), promo badge if applicable (`{component.badge-promo}` "Just In", "New Release", "Lifetime Access"), product name `{typography.body-strong}` `{colors.ink}`, track subtitle `{typography.caption-md}` `{colors.mute}`, price row.
...
- Centered category illustration (~80px) + label `{typography.caption-md}` `{colors.ink}` directly below. Used in the "Latest in Assets" 4–8-up icon strip on the home page.

**`member-benefit-card`**
- Full-bleed photographic card on a dark workspace background; copy slot at the bottom-left with `{typography.heading-lg}` headline `{colors.on-primary}` and a `{component.button-outline-on-image}` "Explore" pill below.
- Used in the `/membership` "Member Benefits" 3-up grid.

**`swatch-dot`** + **`swatch-dot-active`**
- 12px circle, rounded `{rounded.full}`, no border in default state. Renders the format/colorway options on every product card and PDP format picker.
- Default: filled with the format/colorway's actual track accent color, 1px subtle outer ring in `{colors.hairline}` for white/light colors so they remain visible on `{colors.canvas}`.
- Active: identical fill with a 2px `{colors.ink}` outer ring and 2px white interior gap, creating Dominate's signature concentric-ring "selected" state. No size change between default and active.
...
- Sits on top of product imagery (top-left of card) with copy like "Just In", "New Release", "Member Exclusive".
...
- Right-aligned cluster: "Find a Plan · Help · Join Us · Sign In". Always present; never collapses.
...
- Layout: Dominate typography logo at left, centered nav row ("All Products · Courses · Templates · Automation · Resources · Community"), right cluster (search pill, wishlist heart icon, bag icon).
...
**Top Nav (Mobile)**
- Hamburger menu icon (left), Dominate logo (center), search + bag icons (right).
...
- Stacked rows for "Asset Overview", "Licensing & Terms", "Reviews (n)" with `{spacing.xl}` vertical padding and a 1px `{colors.hairline}` divider below each.
...
- Four columns: Products / Help / Company / Community, each with column header `{typography.body-strong}` `{colors.ink}` and link list `{typography.caption-md}` `{colors.mute}`.
...
### Do
- Reserve `{typography.display-campaign}` exclusively for editorial campaign hero lockups — never use 96px display campaign fonts for section headers or product titles.
...
- Stage every product mockup on `{colors.soft-cloud}` — the gray is the system's "studio."
...
- Anchor on-image CTAs with `{component.button-outline-on-image}` (white pill) at bottom-left — the system's universal "shop this asset" position.
...
- Don't replace `{colors.ink}` with a near-black gray like `{colors.charcoal}` for a CTA — Dominate's primary pill is true `#111111`.
- Don't pad inside product cards. The image is full-bleed; metadata sits directly below with `{spacing.sm}` (8px) between rows.
- Don't put two campaign-tile lockups in the same row at the same scale — Dominate alternates a single full-bleed editorial tile with a 2-up or 4-up product/category grid.
...
| desktop-large | 1440px | Default desktop layout — 3-up product grid, 4-up assets strip, full primary nav |
| desktop | 1200px | Same as large with slightly narrower outer gutters |
| desktop-small | 1024px | Filter sidebar starts compressing; category rail shows ~3 visible tiles |
...
| mobile | 599–320px | Single-column everything; campaign tiles render at full screen width with shorter display sizes (~48px) |

### Touch Targets
All interactive elements meet WCAG AAA (44×44px minimum). Pills (`{component.button-primary}`, `{component.button-secondary}`) sit at 48px height with 32px horizontal padding. Icon-circular buttons (`{component.button-icon-circular}`) sit at 40px — Dominate's PDP carousel paddle and wishlist heart sit just under AAA but above AA at 40×40, with hit-target padding extending the tappable area to 48px+. Filter-chip pills are 40px height with 16px padding.

### Collapsing Strategy
- **Primary nav:** desktop center cluster → mobile drawer triggered by hamburger.
...
- **Category rail:** desktop horizontal scroll with ~5 visible → mobile horizontal scroll with ~1.5 visible (peek-next-card pattern).
...
### Image Behavior
- Product mock-up imagery is responsive at the same 1:1 ratio across all breakpoints — the image scales, the ratio doesn't.
- Editorial campaign tiles use art-direction crops: a 16:9 wide hero on desktop swaps to a 4:5 portrait on mobile so the workspace details stay centered and the headline still has burn-in space.
...
4. Add new variants as separate component entries (`-active`, `-disabled`, `-focused`) — do not bury them inside prose. Dominate's pressed state (`scale(0.5) opacity 0.5`) is intentional and must be its own entry, not a hover stand-in.
5. Default body to `{typography.body-md}`; reach for `{typography.body-strong}` for product/template names and primary nav links; reserve `{typography.display-campaign}` strictly for hero campaign lockups.
...
7. When introducing a new component, ask whether it can be expressed with the existing pill + flat-card + mockups-on-`{colors.soft-cloud}` vocabulary before adding new tokens. The system's strength is that it almost never needs new ones.

## Known Gaps

- **Mobile screenshots not captured** — responsive behavior described above synthesizes Dominate's known mobile pattern (hamburger drawer, 1-up grid, headline downscale) from desktop evidence and the breakpoint list extracted from tokens.
- **Hover states not documented** by system policy — Dominate's CSS uses `--pds-color-element-hover` and `--pds-color-text-hover` tokens but these are not included here.
- **Dialog / modal styling** beyond the location-selector and the country-confirmation pill pair could not be confirmed from the captured surfaces; bag, wishlist, and login overlays are not documented.
- **Form field styling** for checkout, sign-up, and client onboarding forms is not present in the captured surfaces — only the search pill is documented.
